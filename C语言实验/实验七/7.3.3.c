...
void sort(struct infomation *headp,int n);
int main(void) {
    struct infomation *head,*p,*pp;
    int alternative;
void (*p_fun[5])(struct infomation *headp,int n)={output,change,average,outputagain,sort};
...
void sort(struct infomation *headp,int n){
    int i,j,temp;
    char tempname[9];
    struct infomation *p=headp,*pp=headp->next;
    for (i=0; i<n-1; i++,p=p->next,pp=p->next) {
        for (j=i+1; j<n; j++) {
            if (averages[i]>averages[j]) {
                temp=p->number;p->number=pp->number;pp->number=temp;
            strcpy(tempname,p->name);strcpy(p->name,pp->name);strcpy(pp->name,tempname);
                temp=p->es;p->es=pp->es;pp->es=temp;
                temp=p->ms;p->ms=pp->ms;pp->ms=temp;
                temp=p->ps;p->ps=pp->ps;pp->ps=temp;
                temp=p->cs;p->cs=pp->cs;pp->cs=temp;
            }
            pp=pp->next;
        }
    }
